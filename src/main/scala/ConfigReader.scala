...
object ConfigReader {
  ...
  val consumerConfig: ConsumerConfig = ConsumerConfig(Server(serverKey, serverValue), KeyDeserializer(key_deserializerKey, key_deserializerValue),
    ValueDeserializer(value_deserializerKey, value_deserializerValue), GroupId(groupIdKey, groupIdValue))
...
case class ConsumerConfig(server: Server, keyDeserializer: KeyDeserializer, valueDeserializer: ValueDeserializer, groupId: GroupId)
...
case class EmailConfig(hostName: String, smtpPort: Int, userName: String, password: String, sslOnCon: Boolean, from: String, subject: String)